...
The XLSX must have a header row with the following column names (column order does not matter — headers are matched by name):
...
|----------------|-------------|---------|
| `ISP/Provider` | Provider name. Defaults to **"Globe"**. | `Globe` |
...
|-------|------|-------------|
| `providerName` | `string` | Name of the provider created/found (e.g. `"Globe"`). |
| `providerCreated` | `boolean` | `true` if the provider was newly created; `false` if an existing provider with the same name was reused. |
...
| `accountsCreated` | `integer` | Number of new accounts created in this import. |
| `accountsReused` | `integer` | Number of accounts that already existed (matched by `providerId` + `accountNumber`) and were reused. |
...
| `400` | Missing required columns | `{"result":"error","message":"missing required column 'Store Code' in header row","status":"400","data":null}` |
...
1. **Provider** — matched by name. If a provider named "Globe" already exists, it is reused (no duplicate).
2. **Stores** — matched by `branchCode` (the Store Code column). If a store with that branch code exists, it is reused. Same store code appearing multiple times (different service types) creates the store once and links multiple accounts.
...
- **Invoice sequence:** when the provider is created for the first time, its `invoice_sequences` row is seeded automatically.
...
curl -X POST http://localhost:8080/accounts/bulk-import \
  ...
  -F "file=@/path/to/Globe Accounts.xlsx"
```
